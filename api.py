...
async def analyze_reviews(request: AnalysisRequest):
    ...
    # --- CHANGED: Hardcoding your specific file instead of scraping ---
    # timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    dataset_name = "google_maps_reviews_2026-01-31_14-59-57.csv" 
    dataset_path = os.path.join("datasets", dataset_name)
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # SCRAPER DISABLED: Using existing file
    ...
    if not os.path.exists(reviews_path):
        return {"error": f"Analysis file not found at {reviews_path}"}
...
async def compare_models(request: ComparisonRequest):
    ...
    # --- CHANGED: Hardcoding your specific file ---
    dataset_name = "google_maps_reviews_2026-01-31_14-59-57.csv"
    dataset_path = os.path.join("datasets", dataset_name)
    base_dir = os.path.dirname(os.path.abspath(__file__))
